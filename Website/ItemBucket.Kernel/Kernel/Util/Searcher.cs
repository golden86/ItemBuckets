...
namespace ItemBucket.Kernel.Kernel.Util
{
   public class IndexSearcher : IDisposable
   {
      ...
      public virtual List<SitecoreItem> RunQuery(Query query)
      {
         return RunQuery(query, false);
      }

      public virtual List<SitecoreItem> RunQuery(Query query, bool showAllVersions)
      {
         Assert.ArgumentNotNull(Index, "ItemSearch");
         ...
         {
            Log.Error("ItemSearch. There was a problem while running a search query. Details: " + exception.Message, this);
      ...
      public virtual List<SitecoreItem> GetItemsInRange(NumericRangeSearchParam param)
      {
         var globalQuery = new CombinedQuery();
